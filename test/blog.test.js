...
// Аудит каталога 2026-08-14 разложил товары по тирам риска: автомарки,
// спортклубы, кино и игры — «снимать немедленно», музыкальная тематика —
// «снимать». Блог о таком товаре хуже самого товара: это реклама спорного
// изделия на своём домене, под именем мастерской и с телефоном, и она остаётся
// в индексе навсегда. Список неполный по определению — он ловит не всё, а самое
// частое, и существует затем, чтобы вопрос вообще был задан.
const RIGHTS_MARKERS = [
  'цой', 'кино', 'король и шут', 'кукрыниксы', 'горшенёв', 'горшенев', 'ария',
  'rammstein', 'metallica', 'nirvana', 'михаил круг', 'высоцк', 'гражданская оборона',
  'сектор газа', 'би-2', 'ддт', 'сплин', 'пикник', 'алиса',
  'bmw', 'audi', 'mercedes', 'toyota', 'lada', 'порше', 'porsche', 'ferrari',
  'спартак', 'цска', 'зенит', 'локомотив', 'динамо', 'реал мадрид', 'барселон',
  'marvel', 'disney', 'звёздные войны', 'звездные войны', 'гарри поттер',
  'человек-паук', 'бэтмен', 'смешарики', 'ну погоди',
];

test('no post advertises a product from the take-down tiers', () => {
  for (const post of POSTS) {
    ...
    for (const marker of RIGHTS_MARKERS) {
      assert.ok(!haystack.includes(marker),
        `${post.slug}: «${marker}» — чужая интеллектуальная собственность, `
        + 'такой товар по аудиту подлежит снятию, и писать о нём в блоге нельзя');
    }
  }
});
